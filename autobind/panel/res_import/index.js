 ...
 * 
 * ** */
const unzipExe = Editor.url('packages://' + packageName + '/tools/unzip.exe');
let Tool = Editor.require('packages://' + packageName + '/tools/tool.js');


Editor.Panel.extend({
  // css style for panel
  style: fs.readFileSync(Editor.url("packages://" + packageName + "/panel/res_import/index.css"),'utf-8'),

  // html template for panel
  template: fs.readFileSync(Editor.url("packages://" + packageName + "/panel/res_import/index.html"),'utf-8'),
      ...
      methods: {
        ...
        async startImport() {
          ...
          if(this.targetPath && this.resPath) {
            ...
            /** 判断平台 */
            let resPath = this.changePath(path.join(Editor.url("packages://" + packageName + "/tools"),'/unzip.exe'));
            let tempPath = "";
